...
import { buildSkillManifest, createToolPermissionGuard, loadSkillManifest } from "../src/index";
...
describe("buildSkillManifest", () => {
  ...
  test("重複するアプリ名でもファイルパスは重複しない", async () => {
    const result = await buildSkillManifest({
      applications: ["Photoshop", "photoshop", "PHOTOSHOP"],
    });
    expect(result.isOk).toBe(true);
    if (!result.isOk) return;
...
describe("createToolPermissionGuard", () => {
  test("skills/配下へのWriteはallowされる", async () => {
    ...
    const result = await guard(
      "Write",
      { file_path: "packages/core/skills/tools/photoshop/new.md" },
      GUARD_OPTIONS,
    );
  ...
  test("パストラバーサルを含むWriteはdenyされる", async () => {
    ...
    const result = await guard(
      "Write",
      { file_path: "packages/core/skills/../src/index.ts" },
      GUARD_OPTIONS,
    );
    expect(result.behavior).toBe("deny");
  });
  ...
  test("skills/配下のReadはallowされる", async () => {
    ...
    const result = await guard(
      "Read",
      { file_path: "packages/core/skills/techniques/masks.md" },
      GUARD_OPTIONS,
    );
    expect(result.behavior).toBe("allow");
  });
  ...
  test("extract-video.tsの正規実行はallowされる", async () => {
    ...
    const result = await guard(
      ...
      {
        command:
          "bun run packages/core/src/extract-video.ts https://www.youtube.com/watch?v=abc123",
      },
      GUARD_OPTIONS,
    );
    expect(result.behavior).toBe("allow");
  ...
  test("コマンドインジェクション(セミコロン)はdenyされる", async () => {
    ...
    const result = await guard(
      ...
      {
        command:
          "bun run packages/core/src/extract-video.ts https://youtube.com/watch?v=x; rm -rf /",
      },
      GUARD_OPTIONS,
    );
    expect(result.behavior).toBe("deny");
  ...
  test("不正なURL引数はdenyされる", async () => {
    ...
    const result = await guard(
      "Bash",
      { command: "bun run packages/core/src/extract-video.ts https://example.com/malicious" },
      GUARD_OPTIONS,
    );
    expect(result.behavior).toBe("deny");
  });
  ...
  test("skills/配下のGlobはallowされる", async () => {
    ...
    const result = await guard(
      "Glob",
      { path: "packages/core/skills/tools/photoshop" },
      GUARD_OPTIONS,
    );
  ...
  test("URL引数なしでもコマンド構造が正しければallowされる", async () => {
    ...
    const result = await guard(
      "Bash",
      { command: "bun run packages/core/src/extract-video.ts" },
      GUARD_OPTIONS,
    );
  ...
  test("有効な動画ID文字列のみで構成されたURLはallowされる", async () => {
    ...
    const result = await guard(
      ...
      {
        command:
          "bun run packages/core/src/extract-video.ts https://www.youtube.com/watch?v=abc123EXTRA",
      },
      GUARD_OPTIONS,
    );
  ...
  test("YouTube URL末尾にスペースで追加ペイロードがある場合denyされる", async () => {
    ...
    const result = await guard(
      ...
      {
        command:
          "bun run packages/core/src/extract-video.ts https://www.youtube.com/watch?v=abc123 --dangerous-flag",
      },
      GUARD_OPTIONS,
    );

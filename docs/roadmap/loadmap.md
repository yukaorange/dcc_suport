...
> **注記（2026-03-19）**: 実装では Claude Agent SDK を直接使用する方式に変更し、セッション管理は SQLite で永続化している。以下の Hooks 構想は実装されておらず、DB 永続化で代替済み。
...
### Claude Agent SDK（ライブラリ呼び出し）

仕様策定時は Claude Code CLI をサブプロセスで呼び出す方式を想定していたが、実装では **Claude Agent SDK**（`@anthropic-ai/claude-agent-sdk`）をライブラリとして直接呼び出す方式を採用した。

- **API 従量課金**（Anthropic API キーが必要）
- `engine.ts` の `invokeClaude()` 関数が SDK の `query()` を呼び出す
- ストリーミングでメッセージを逐次処理
- 排他制御（ミューテックスロック）で並列呼び出しを防止
- タイムアウト管理（最大300秒）

### セッション継続

SDK のセッション ID で会話を継続する。`checkSessionContinuity()` でセッション喪失を検知し、自動復旧する。
...
SDK の agents 機能で advisor / researcher を定義。親エージェントは `Agent` ツールのみを持ち、サブエージェントに委譲する。

### Web検索

researcherサブエージェントの多段探索の第2段階。SDK の WebSearch / WebFetch ツールでブログ記事を取得・構造化する。

### Gemini API連携（YouTube動画の構造化抽出）

researcherの多段探索の第3段階として、YouTube動画の内容をGemini APIで構造化抽出する。

- **位置づけ**: researcher が Bash ツール経由で `bun run extract-video.ts <URL>` を実行
...
- **Bun workspaces** によるモノレポ構成

### ライブラリ

| 領域 | パッケージ | 用途 |
|---|---|---|
| AI連携 | **@anthropic-ai/claude-agent-sdk** | Claude API との通信、サブエージェント構成 |
| AI連携 | **@google/genai** | YouTube動画の構造化抽出（Gemini 2.5 Flash） |
| スクリーンキャプチャ | **screenshot-desktop** | Win/Mac/Linux対応、OS標準コマンドのラッパー |
| ピクセル差分検知 | **pixelmatch** | 前回画像との変化検知（トリガー用途のみ） |
| 画像リサイズ | **sharp** | トークン節約のためキャプチャを縮小 |
| Webサーバー | **Hono** | 軽量Webフレームワーク。Bun.serve() で起動 |
| API | **tRPC v11** | 型安全なRPC。Hono fetchアダプタ経由 |
| DB | **drizzle-orm** + **bun:sqlite** | セッション・プラン・アドバイスの永続化（WALモード） |
| フロントエンド | **React 19** + **Vite** | SPA。開発時はVite proxy経由でHonoと通信 |
| UIコンポーネント | **shadcn/ui** + **Radix UI** | アクセシブルなUIプリミティブ |
| 状態管理 | **TanStack Query** | tRPC React統合でサーバー状態を管理 |
| CLIプロンプト | **@inquirer/prompts** | CLI版のインタラクティブ入力 |
| バリデーション | **zod** | tRPCの入力スキーマ定義 |
| リント | **Biome** | ESLint + Prettier の代替（フォーマット + リント） |
| テスト | **vitest** | ユニットテスト（core / cli） |
| テスト | **bun:test** | サーバーパッケージのテスト |
| E2Eテスト | **Playwright** | ブラウザ自動テスト（Chromium） |
...
- `bun install` → `bun run start` で起動

---

## ディレクトリ構成

※ 以下は仕様策定時の構想。実際のモノレポ構成は [architecture.md](../architecture.md) を参照。

```
dcc_suport/
├── packages/
│   ├── core/              # 共有ドメインロジック（AI・キャプチャ・プラン・差分検知）
│   │   ├── src/           # engine, coach-loop, planner, capture, diff, prompts, skills, agents, ...
│   │   ├── test/          # vitest ユニットテスト
│   │   ├── skills/        # スキルファイル（techniques/ + tools/）
│   │   └── src/verify/    # 手動検証スクリプト
│   ├── server/            # Webサーバー + API + DB
│   │   ├── src/db/        # Drizzle ORM スキーマ・CRUD
│   │   ├── src/lib/       # coach-session, start-session, image-store
│   │   ├── src/pure/      # event-bus, pending-plan-cache
│   │   ├── src/trpc/      # tRPC ルーター（session, plan, setup, display, events）
│   │   └── test/          # bun:test DB テスト
│   ├── client/            # React SPA（Vite + TanStack Query）
│   │   ├── src/components/  # setup/, dashboard/, session/, shared/, ui/
│   │   ├── src/hooks/     # useLoopEvents（SSE購読）
│   │   └── src/lib/       # ユーティリティ
│   └── cli/               # CLI版エントリポイント
│       └── src/           # index.ts, setup-flow.ts
├── e2e/                   # Playwright E2Eテスト
├── docs/                  # ドキュメント
├── .github/workflows/     # CI/CD（typecheck + lint + test）
├── biome.json             # リント・フォーマット設定
├── playwright.config.ts   # E2Eテスト設定
├── vitest.config.ts       # ユニットテスト設定
└── package.json           # モノレポルート（workspaces, catalog, scripts）
```
...
| フィールド | 範囲 | デフォルト | 説明 |
|-----------|------|----------|------|
| intervalSeconds | 1-300 | 5 | ループ間隔（秒） |
| diffThresholdPercent | 0-100 | 5 | AI呼び出しの画面変化率閾値 |
| maxImageWidthPx | 320-7680 | 1280 | キャプチャ画像の最大幅 |
...
※ 仕様策定時は Claude Code CLI 経由を想定していたが、実装では Claude Agent SDK を直接使用する方式に変更した。以下の検証事項の多くは SDK 採用により解決済み。

1. ~~`claude -p` で画像ファイルパスを読めるか~~ → **解決済み**: SDK の query() で画像パスを直接渡せる
2. ~~`-c -p` の同時使用でセッション継続が機能するか~~ → **解決済み**: SDK のセッション ID で管理。`checkSessionContinuity()` で喪失検知・復旧
3. ~~`-p` モードで `--agents` が動作するか~~ → **解決済み**: SDK の agents オプションで定義
4. ~~`-p` モードでHooksが発火するか~~ → **方針変更**: DB 永続化で代替。Hooks は未使用
5. ~~Claude CodeのWeb検索ツール~~ → **解決済み**: SDK の WebSearch / WebFetch ツール
6. ~~プランのデータ構造~~ → **解決済み**: `Plan { goal, referenceSummary, steps: PlanStep[] }` で定義済み
7. **API レート制限** — Anthropic API の従量課金。長時間セッションでのコスト・レート制限は引き続き要確認
...
- [x] screenshot-desktopでディスプレイ一覧表示＋キャプチャが動くことを確認
- [x] pixelmatchで差分検知が動くことを確認
- [x] Claude Agent SDK で画像ファイルパスを渡して応答が返ることを確認
- [x] サブエージェント（agents）の動作確認
- [x] 上記を組み合わせた最小ループの動作確認

### Phase 2: コアループ実装 ✅ 完了

- [x] 設定ファイル読み込み
- [x] ディスプレイ選択UI（CLI: inquirer / GUI: ドロップダウン）
- [x] リファレンス画像指定＋目標記述 → プラン生成
- [x] 自律監視ループ（5秒間隔、差分検知、MessageBox パターン）
- [x] システムプロンプト（コーチモード＋介入判断ロジック）
- [x] ターミナル出力のフォーマット

### Phase 3: スキルファイル＋サブエージェント＋Gemini連携 ✅ 完了

- [x] スキルファイル2層構造（techniques / tools）のディレクトリ構築
- [x] スキルファイルのシステムプロンプト注入（目次のみ。中身は researcher が Read）
- [x] advisor / researcher サブエージェント定義
- [x] researcherの多段探索フロー実装（スキルファイル → ブログ → YouTube）
...
- [x] モノレポ構成への移行（@dcc/core, @dcc/server, @dcc/client, @dcc/cli）
- [x] Hono + tRPC サーバー（localhost:3456）

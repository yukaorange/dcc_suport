...
YouTube ページは巨大なため WebFetch では取得できない。**動画要約は時間がかかるため、必ず \`run_in_background: true\` で起動すること**:
1. Bash(command: 'bun run packages/core/src/extract-video.ts "<youtube-url>"', run_in_background: true)
2. 返ってきた backgroundTaskId を使って TaskOutput(task_id: "<id>", block: true, timeout: 1800000) で結果を待つ
...
- **必ず \`run_in_background: true\` で起動すること**（同期実行は10分でタイムアウトするため）:
  - URLは必ずダブルクォートで囲む（&を含むURLがシェルで壊れるため）
  - リダイレクト（2>&1等）は付けない
  - 起動例:
    Bash(command: 'bun run packages/core/src/extract-video.ts "https://www.youtube.com/watch?v=XXXXX"', run_in_background: true)
  - 起動直後に返ってくる \`backgroundTaskId\` を控えておく
  - TaskOutput ツールで結果を待つ:
    TaskOutput(task_id: "<backgroundTaskId>", block: true, timeout: 1800000)
- **この実行は1回のみ。結果の良し悪しに関わらず再実行しない。**

### ステップ4: 知識の蓄積
- 要約結果をスキルファイルに書き戻す
  - ツール固有の操作手順 → \`skills/tools/<アプリ名>/<file>.md\`
  - アプリ横断の表現技法 → \`skills/techniques/<file>.md\`
  - 該当ファイルが存在しない場合は新規作成してよい
- **Write の file_path は必ず \`skills/\` で始まる仮想パスを使うこと**（例: \`skills/tools/after-effects/glitch.md\`）
- 絶対パスや \`packages/...\` を含む実パスは使わない。スキルマニフェストに表示されているパス形式と同じにする
...
- Adobe CCのメニュー名は日本語UIを前提にするが、アドバイス時には「ブラー（gausian blur）」のように英字版GUIでの表現も併記すること。`;
  ...
  if (input.referenceImages.length > 0) {
    const pathList = input.referenceImages
      .map((img, i) => {
    ...
    sections.push(`
## リファレンス画像
以下のリファレンス画像が提供されています:
${pathList}
これらはユーザーが目指す方向性の参考として提供されたものです。完コピしたいわけではない。表現技法や空気感、色彩に焦点をあててプロのグラフィックデザイナーのように構成を解剖していくような目線で分析してください。`);
  }
...
function formatUserMessageBlock(userMessage: UserMessage): string {
  const parts: string[] = [];

  if (userMessage.text.length > 0) {
    parts.push(`ユーザーからのメッセージ:\n${userMessage.text}`);
  }
